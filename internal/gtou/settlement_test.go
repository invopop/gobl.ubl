...
func TestNewSettlement(t *testing.T) {
	t.Run("should contain the transaction settlement", func(t *testing.T) {
		...
		assert.Equal(t, gtoc.TypeCodeInstrumentNotDefined, doc.Transaction.Settlement.TypeCode)
	...
	t.Run("should set ReferencedDocument for correction invoice", func(t *testing.T) {
		...
		assert.Equal(t, &gtoc.Date{Date: "20240213", Format: "102"}, doc.Transaction.Settlement.ReferencedDocument.IssueDate)
	})
}

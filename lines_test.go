...
func TestNewLines(t *testing.T) {
	t.Run("invoice-without-buyers-tax-id.json", func(t *testing.T) {
		...
		assert.Equal(t, "18.00", doc.InvoiceLines[0].AllowanceCharge[0].Amount.Value)
		...
		assert.Equal(t, "18.00", doc.InvoiceLines[0].AllowanceCharge[1].Amount.Value)
		assert.Equal(t, "1", *doc.InvoiceLines[0].AllowanceCharge[1].MultiplierFactorNumeric)

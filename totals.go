	...
	if t.Taxes != nil && len(t.Taxes.Categories) > 0 {
		for _, cat := range t.Taxes.Categories {
			for _, r := range cat.Rates {
				...
				if r.Ext.Get(untdid.ExtKeyTaxCategory).String() != "" {
					k := r.Ext.Get(untdid.ExtKeyTaxCategory).String()
					taxCat.ID = &k
				}
				if r.Ext.Get(cef.ExtKeyVATEX).String() != "" {
					v := r.Ext.Get(cef.ExtKeyVATEX).String()
